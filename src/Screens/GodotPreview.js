        ...
        async function getThemeDetails(themeId) {
            ...
            if (Object.keys(json).length > 0 && json['themesMap'] !== null && json['themesMap'] !== undefined) {
                ...
                if (themesMap && themesMap[themeId]) {
                    ...
                    let { gameFile } = acessObj
                    if (gameFile && Object.keys(gameFile).length > 0) {
                        let splitDot = gameFile.fileName.split(".")
                        if (splitDot.length > 0) {
                            let docsId = splitDot[0];
                            setGodDotDocsId(docsId)
                            setScript(docsId);
                        }
                    }
                }
...
export default withRouter(GodotPreview)
